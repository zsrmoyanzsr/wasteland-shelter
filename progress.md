...
## v2.3.1 经济平衡修复: 加废料场 + 提高派遣废铁奖励 (本轮)

> 修复长测发现的后期经济瓶颈(废铁/零件枯竭卡死发展)。**5套测试全过,长测废铁从枯竭变稳定。**

### 问题回顾(v2.3长测发现)
- scrap(废铁)是**唯一没有稳定产出设施**的资源:工坊产 parts 反而消耗 scrap,发电机也消耗 scrap
- 唯一来源是派遣(有冷却+风险),导致后期 scrap 长期枯竭,卡死所有升级/建造
- 对比: 食物/水/电/药都有对应产出设施且爆仓,唯独废铁无设施产出

### 修复1: 新增「废料场」设施 (facilities.js)
- ♻️ 废料场: 稳定产 scrap 0.35/s,小耗 power 0.08/s,2 工作位,工程师特长加成
- 成本低(8 scrap + 3 parts),开局即可建,快速回本
- 归入 production 类(生产),与发电机/工坊并列
- **零改引擎**: FACILITY_TYPES 加一个就自动出现在建造菜单,UI/经济/测试全适配(印证架构可扩展性)

### 修复2: 提高派遣 scrap 奖励 (regions.js)
- town: scrap 4-9 → **7-14** (+78%)
- factory: scrap 2-6 → **5-11** (+83%)
- hospital: scrap 2-6 → **4-9** (+50%)
- military: 新增 scrap 6-13
- cache: scrap 1-4 → **3-7** (+75%)
- 派遣成为废铁的补充来源(配合废料场),而非唯一救命稻草

### 长测验证(test_balance.mjs)
| 资源 | v2.3(修复前) | v2.3.1(修复后) |
|------|------------|--------------|
| 废铁 | **0(枯竭,卡死)** | **满仓运转** ✅ |
| 零件 | 3(短缺) | 8→满仓 ✅ |
| 设施建设 | 3个(卡住) | 5个(持续) ✅ |
| 基地升级 | Lv1(卡死) | 可发展 |
- 500天长测从"废铁枯竭无法发展"变"资源稳定产出,发展不再卡死"

### 已知遗留(中后期内容,非平衡bug)
- 修复后 food/water/power/meds 后期会爆仓(产出>消耗),因缺乏中后期消耗出口
- 这需要科技树/更多建筑/新机制解决,属于内容深度问题,非数值平衡问题
- 现状可接受: 前中期发展顺畅,后期满仓不影响"玩通"
...
## v3.0-v4.2 内容大扩展 + 新系统 (本轮)

> 6轮迭代,从v2.6扩展到v4.2,新增7大系统+大量内容。7套测试共190+项全过。

### v3.0 物品系统 + 科技树 (长线扩展地基)
- **物品系统**: content/items.js(17种4分类) + engine/inventory.js(背包增删查/消耗品使用)
- **科技树**: content/tech.js(3线×5级) + engine/techEngine.js(研发扣资源/加成/结局触发)
- 科技满级解锁结局(warlord/civilization/science)
- 存档迁移v3→v6(预留inventory/tech/equipment/prestige字段)

### v3.1 装备系统 → v3.2 改为神器系统
- 原装备系统(通用武器/护甲/配饰)废弃,改为**神器系统**
- **18种神器**: C精良5/B稀有4/A史诗4/S传说5,存档内唯一
- **SABC级**: 按danger加权随机掉落(danger1=3%/danger4=15%)
- **槽位**: 综合属性≥25→2槽,否则1槽;穿戴有等级门槛
- 神器属性计入派遣产出
- 居民详情模态:神器槽位穿戴/卸下

### v3.3 威胁系统 (消耗道具应对,不致死)
- 4种威胁: 辐射风暴(辐射清除剂)/瘟疫(抗生素)/大规模入侵(兴奋剂)/极寒寒潮(止血包)
- 道具应对: 消耗道具完全防护;硬扛:扣血到1不致死+扣资源+心情降
- 每12-18天触发,后期威胁更多

### v3.4 虚弱debuff体系
- 三档: 健康(≥60)正常/轻伤(30-60)设施-20%派遣-15%/重伤(<30)设施-40%不能派遣
- 血量条三档变色 + 🩸重伤/💢轻伤标识

### v4.0 内容大扩展
- **结局系统**: 4结局(战争领主/重建文明/方舟计划/末日真相)+全屏结局画面
- **3张新地图**(5→8): 地下水道/秘密实验室/核战地堡
- **2种新POI**: 补给站(supply)/实验室(lab)
- **2新职业**(6→8): 科学家(研究员)/猎人(追踪者)
- **名字池扩充**: FIRST_NAMES 20→35, NICKNAMES 10→20
- **4新成就**(10→14): 神器收藏家/废土丈量者/全能所长
- **世代传承**: 达成结局→永久+5%产出/结局,开始屏显示"第N代所长"

### v4.1 地图放大 + 建筑扩展 + 事件降低
- 地图放大约50%: home 8×7→12×10, 最大16×13(208格), 总1358格
- 事件密度 33%→15-20%(大地图不烦)
- 3种新建筑(7→10): 温室大棚/瞭望塔/军械工坊

### v4.2 地图特殊地标 + 蓝图解锁建筑 + 背包/解雇
- **8种特殊地标**(25个散布8张地图): 废弃图纸(解锁蓝图)/资源矿脉/隐藏避难所(招募)/神秘祭坛(全队技能+1)/信号塔/宝箱/基因舱/核弹坑(稀有材料+神器)
- **4种蓝图解锁建筑**(10→14): 水力发电机/生物实验室/反物质反应堆/自动化农场
  - 只能通过地图地标发现图纸,发现前建造菜单不显示
- **背包消耗品使用**: 点击消耗品→选幸存者→回血
- **解雇幸存者**: 详情模态解雇按钮+确认模态+装备归还+最后1人保护
...
### 测试体系
- test_logic(26) + test_edge(14) + test_deep(65) + test_ui_e2e(27) + test_balance(长测) + test_full_audit(32) + test_mega(26)
- 全程0 console error
