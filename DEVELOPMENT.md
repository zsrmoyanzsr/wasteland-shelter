...
16. **游戏主循环必须包 try/catch** — v2.5.1 修：render/update 任何异常都会中断 requestAnimationFrame，导致游戏永久冻结、所有输入失效。游戏循环的 try/catch 是最后一道防线，绝不能省。
17. **外部数据入口都要 null 守卫** — v2.5.1 修：poiInfo() 对无效 regionType 返回 null，6 处直接解引用 info.icon 崩溃。老存档/数据污染/迁移异常都可能产生非法值，凡是从存档/外部读入再解引用的，都要兜底。
18. **数值平衡要测长周期** — v2.5.1 修：袭击攻击 8+day*1.5 线性无上限，81 天后防御必然击穿，但单元测试只跑短周期发现不了。教训：数值公式要跑 300/600 天长测验证收敛性，发散的线性增长必爆。
19. **多分支结算要对齐且不重复** — v2.5.1 修：resolveEvent 对负 reward 先 applyRewards 又循环扣一次（双重扣减）。教训：applyRewards 应是唯一的资源变更入口（带 Math.max(0) 下限），别在外面再手动扣。
20. **派真玩家(playtest)才能发现体验问题** — v2.5.1 派 4 个子 agent 各玩 600+ 天，发现了只看代码发现不了的问题（袭击后期失效、stats 永远 0）。教训：自动化 playtest（模拟真实玩家混合操作长跑）比单元测试更能暴露集成 bug。
